...
- MCP: pi reads `mcpServers` from `~/.pi/agent/mcp.json` (global) and `.mcp.json` at the project root (confirmed as a real pi-mcp-adapter config layer — its README lists the full precedence chain). Decision: use `.mcp.json` at the project root — tool-agnostic, so any harness (pi, Copilot, Cursor, etc.) can use it. pi-mcp-adapter reads it as a normal project config layer.
